...
public class CrossingGate extends Observable implements Observer {
	...
	public void update(Observable o, Object arg) {
		if (o instanceof Train) {
			...
			// Handle state changes for both train directions
			switch (train.getDirection()) {
			case WEST:
				if (train.getVehicleX() < exitPoint + 20 && train.getVehicleX() > exitPoint) { // leave station if out of range
					currentGateState.leaveStation();
				} else if (train.getVehicleX() < triggerPoint + 20 && train.getVehicleX() > exitPoint + 20) {
					currentGateState.approachStation();
				}
				break;
			case EAST:
				if (train.getVehicleX() > triggerPoint && train.getVehicleX() < triggerPoint + 20) { // leave station if out of range
					currentGateState.leaveStation();
				} else if (train.getVehicleX() > exitPoint - 20 && train.getVehicleX() < triggerPoint) {
					currentGateState.approachStation();
				}
				break;
			default:
