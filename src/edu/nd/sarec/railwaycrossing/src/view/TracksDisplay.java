...
public class TracksDisplay implements IDisplay {
	...
	public void draw() {
		for (RailwayTracks track : tracks) {
			root.getChildren().add(new Line(track.getStartX(), track.getStartY() - trackSize, track.getEndX(),
					track.getEndY() - trackSize));
			root.getChildren().add(new Line(track.getStartX(), track.getStartY() + trackSize, track.getEndX(),
					track.getEndY() + trackSize));
			for (int j = track.getStartX() + (trackSize / 2); j < track.getEndX(); j += trackSize) {
				root.getChildren()
						.add(new Line(j, track.getStartY() - trackSize - 2, j, track.getEndY() + trackSize + 2));
			}
		}
	}
}
